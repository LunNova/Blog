...
(function() {
  ...
  // Grab all h2/h3 with IDs (your § anchors already give them IDs, right?)
  const headers = document.querySelectorAll('h2[id], h3[id]');
  if (headers.length === 0) return; // No headers? Bail.
  ...
  headers.forEach(h => {
    ...
    li.className = h.tagName.toLowerCase(); // 'h2' or 'h3' for styling

    const a = document.createElement('a');
    a.href = `#${h.id}`;
    a.textContent = h.textContent.replace('§', '').trim(); // Strip the § symbol
  ...
  let activeId = null;
	...
	const observer = new IntersectionObserver((entries) => {
		// Get all intersecting entries, sorted by position (topmost first)
		const visibleEntries = entries
			.filter(e => e.isIntersecting)
			.sort((a, b) => {
				// Sort by vertical position - topmost (smallest .top) comes first
